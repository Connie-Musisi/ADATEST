...
# Method: ADATEST
ADATEST_method = function(data){
  res = ADATEST(data, group_var="group",
                group_levels= c("0", "1"), t0=1.5, t1=3, p.adjust.method = "BH",
                empirical_adjust = FALSE)
  res_pval = res$p_value
  
  res_adjP = res$p_adj
  
  res_stat = res$test_statistic
  
  out = data.frame(stat = res_stat, pval = res_pval, adjP = res_adjP) 
  rownames(out) = rownames(res$Original_results$Original_results)
...
# Method: MaAsLin2
MaAsLin2_method <- function(data) {
  # 1) Check that ‘group’ exists
  if (!inherits(data, "phyloseq")) {
    stop("Input must be a phyloseq object.")
  }
  if (!"group" %in% colnames(sample_data(data))) {
    stop("Your phyloseq’s sample_data must include a ‘group’ column.")
  }
  
  # 2) Build feature table: samples × taxa
  feat_df <- if (taxa_are_rows(data)) {
    as.data.frame(t(otu_table(data)))
  } else {
    as.data.frame(otu_table(data))
  }
  #rownames(feat_df) <- sample_names(data)
  
  # 3) Build metadata table with just ‘group’
  meta_df <- as(sample_data(data), "data.frame")[, "group", drop = FALSE]
  meta_df$group <- factor(meta_df$group)
  #rownames(meta_df) <- rownames(sample_data(data))
  
  # 4) Run Maaslin2 into a temporary directory
  out_dir <- file.path(tempdir(), "maaslin2_output")
  ...
  # 5) Read in the all_results.tsv and subset to “group”
  res_file <- file.path(out_dir, "all_results.tsv")
  if (!file.exists(res_file)) {
    stop("Maaslin2 did not produce all_results.tsv")
  }
  all_res <- read.delim(res_file, check.names = FALSE, stringsAsFactors = FALSE)
  
  out <- data.frame(all_res[,c("pval","qval","coef")])
  colnames(out) <- c("pval", "adjP", "stat")
  rownames(out) <- all_res[,"feature"]
  return(out)
}
...
eval<-function(alpha,n.taxa,pval,isDA){
  results<-c()
  for (j in (1:n.taxa)){
