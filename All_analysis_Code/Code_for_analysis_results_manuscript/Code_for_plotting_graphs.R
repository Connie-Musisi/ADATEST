...
# Load data
allresults = read.csv("IBD_main_manuscript_results.csv", sep=";")

# Results from one sample size group
all_res<-allresults[allresults$SampleSize==50,]
...
Results.Summary<-function(all_res, method="ADATEST") {
  ressum<-data.frame(
...
load("~/Results/ADATEST_results.RData")

parest_list <- list()
# Loop over the elements in your dataset (assuming the 'i' indices are defined)
for (i in 1:length(ADATEST_allres_Adjust2Med[["Setting5_n50_b5_rnt61"]])) {
  parest <- ADATEST_allres_Adjust2Med[["Setting5_n50_b5_rnt61"]][[i]][["Train_results"]][["Train_parest"]]
